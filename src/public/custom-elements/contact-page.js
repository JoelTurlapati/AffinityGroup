// ─── Shared helpers (duplicated per Wix file-isolation requirement) ───────────
...
contact-page { display: block; font-family: 'Inter', sans-serif; color: #1a1a1a; }
...
.cp-hero__overlay {
  ...
  background: rgba(14,61,71,0.68);
}
.cp-hero__content {
  position: relative; z-index: 2;
  width: 100%; max-width: 620px;
  margin: 0 auto; padding: 100px 32px 60px;
}
.cp-hero__heading {
...
.cp-form .form-group { margin-bottom: 20px; }
.cp-form label {
  display: block; margin-bottom: 6px;
  font-size: 12.5px; font-weight: 600; color: #145568;
...
.cp-form textarea:focus {
  outline: none;
  border-color: #1D7080;
  box-shadow: 0 0 0 3px rgba(29,112,128,.12);
}
.cp-form textarea { min-height: 110px; }
...
.cp-form__submit:hover { background: #145568; }
.cp-disclaimer {
  margin-top: 18px;
  font-size: 12px; color: #6a8a92; line-height: 1.7;
  text-align: center;
}
...
.cp-info__inner { max-width: 900px; margin: 0 auto; }
...
.cp-step {
  ...
  border: 1.5px solid #E6F4F7;
  border-radius: 10px;
  transition: box-shadow .2s, border-color .2s;
}
.cp-step:hover {
  box-shadow: 0 8px 32px rgba(29,112,128,.12);
  border-color: #1D7080;
}
.cp-step__icon {
  width: 56px; height: 56px;
  background: #E6F4F7; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  margin: 0 auto 20px;
  color: #1D7080; font-size: 22px;
}
.cp-step h3 {
  font-family: 'Playfair Display', serif;
  font-size: 1.15rem; color: #145568;
  margin-bottom: 12px; font-weight: 600;
}
.cp-step p { font-size: .92rem; line-height: 1.75; color: #5a6a6e; }

.cp-info__note {
  margin-top: 48px;
  padding: 28px 36px;
  background: #F0F8FA;
  border-left: 4px solid #1D7080;
  border-radius: 0 8px 8px 0;
  font-size: .95rem; line-height: 1.85; color: #3a4a4e;
}
...
<main>
  ...
  <section class="cp-hero">
    <video autoplay muted loop playsinline>
      <source src="/public/assets/ContactPageBackgroundVideo.mp4" type="video/mp4" />
    </video>
    <div class="cp-hero__overlay"></div>
    <div class="cp-hero__content">
      <h1 class="cp-hero__heading">Getting Started on Your Journey</h1>
      <div class="cp-form">
        <form method="post" action="#" id="contact-form">
          <div class="cp-form-row">
            <div class="form-group">
              <label for="cp-first-name">First Name</label>
              <input type="text" id="cp-first-name" name="first_name" placeholder="Jane" required />
            </div>
            <div class="form-group">
              <label for="cp-last-name">Last Name</label>
              <input type="text" id="cp-last-name" name="last_name" placeholder="Doe" required />
            </div>
          </div>
          <div class="form-group">
            <label for="cp-phone">Phone</label>
            <input type="tel" id="cp-phone" name="phone" placeholder="(330) 555-1234" required />
          </div>
          <div class="form-group">
            <label for="cp-message">Message</label>
            <textarea id="cp-message" name="message" placeholder="Tell us a bit about your situation&hellip;"></textarea>
          </div>
          <button type="submit" class="cp-form__submit">SUBMIT</button>
        </form>
        <p class="cp-disclaimer">
          Please do not send personal financial information through this form.
          If you need to submit forms or access personal information, please call our
          Canton, Ohio office at (330)&nbsp;526-8412.
        </p>
      </div>
    </div>
  </section>
  ...
  <section class="cp-info">
    <div class="cp-info__inner">
      <h2 class="cp-info__headline">No Cost. No Pressure. Always Transparent.</h2>
      <p class="cp-info__sub">We look forward to speaking with you soon.</p>

      <div class="cp-steps">
        <div class="cp-step">
          <div class="cp-step__icon">&#128222;</div>
          <h3>Step 1 &mdash; Intro Call</h3>
          <p>Once you enter your information we will contact you via phone call within 24 hours. This call is free and just introductory &mdash; no financial advice will be given.</p>
        </div>
        <div class="cp-step">
          <div class="cp-step__icon">&#128100;</div>
          <h3>Step 2 &mdash; Advisor Meeting</h3>
          <p>We will then set up a meeting with Financial Advisor and CEO Scott Lowe to start your journey toward your financial goals.</p>
        </div>
        <div class="cp-step">
          <div class="cp-step__icon">&#127760;</div>
          <h3>Nationwide Service</h3>
          <p>Location is no barrier. We service people from all across the country. No form of payment is required for the initial phone call and meeting.</p>
        </div>
      </div>

      <div class="cp-info__note">
        No form of payment required for the initial phone call and meeting. Location is no barrier as we service people from all across the country. We look forward to speaking with you soon.
      </div>
    </div>
  </section>
...
`;
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        const btn = form.querySelector('.cp-form__submit');
        btn.textContent = 'Thank you! We\'ll be in touch within 24 hours.';
        btn.style.background = '#145568';
        btn.disabled = true;
      });
    }
  }
